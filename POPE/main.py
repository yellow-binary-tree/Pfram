...
def main():
    ...
    assert not os.path.exists(os.path.join(save_path, f"{config.dataset}_pope_random.json"))
    ...
    # merge pope questions to keep only different questions
    base_folder = os.path.join(config.save_path, config.dataset)
    output_fname = os.path.join(base_folder, f"{config.dataset}_pope_questions.json")
    shutil.copyfile(os.path.join(base_folder, f"{config.dataset}_pope_random.json"), output_fname)
    question_id = len(open(output_fname, 'r').readlines()) + 1
    f_out = open(output_fname, 'a')
    for split in ['popular', 'adversarial']:
        with open(os.path.join(base_folder, f"{config.dataset}_pope_{split}.json"), 'r') as f:
            for line in f:
